...
1. Explain the difference between `RDBMS` and `SQL`.
  ...
  The RDBM is the system used to manage the database, SQL is the query language used on the RDBM.

1. Why do tables need a `primary key`?

  Tables need primary key's to identify each resource in the database.
...
1. What do we need in order to have a _many to many_ relationship between two tables.
  ...
  We need a joining table that references each foreign foreign key of the tables we want to join.
